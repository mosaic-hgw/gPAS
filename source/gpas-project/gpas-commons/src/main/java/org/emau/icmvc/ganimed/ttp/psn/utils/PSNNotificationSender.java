...
public class PSNNotificationSender extends NotificationSender
{
	...
	{
		return sendNotificationForInsertValuePseudonymPairs(clientId, domainName, List.of(Map.entry(value, pseudonym)), List.of());
	}

	public boolean sendNotificationForInsertValuePseudonymPairs(String clientId, String domainName, List<Map.Entry<String, String>> pairs, List<InsertPairExceptionDTO> result)
	{
		if (StringUtils.isBlank(clientId))
		{
			return false;
