...
{
	@Serial
	private static final long serialVersionUID = -3675340794064390590L;
	 ...
	 * @param useLastCharAsDelimiter true to use the last character as delimiter
	 *
	 * @throws ArithmeticException
	 *             if this number exceeds int
	 * @throws CharNotInAlphabetException if the psn contains characters which are not in the alphabet
	 */
	...
	/**
	 * {@return number of possible psn with this alphabet}
	 *
	...
	/**
	 * {@return the maximum number of bytes required to encode each single symbol of the alphabet when using UTF8MB4 encoding in databases (which can be 1, 2, 3, or 4)}
	 */
	public int getMaxBytesPerSymbolInUtf8mb4()
	{
		...
		{
			int count = Alphabets.utf8mb4ByteCount(symbol);
			if (count > max)
			{
				max = count;
			}
