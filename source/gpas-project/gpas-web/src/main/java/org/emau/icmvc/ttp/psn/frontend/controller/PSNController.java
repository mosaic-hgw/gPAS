...
@ViewScoped
@Named( "psnController")
public class PSNController extends AbstractGPASBean implements Serializable
{
	@Serial
	private static final long serialVersionUID = 5074099965536718831L;
	private PSNDTOLazyModel psnLazyModel;

	private DomainOutDTO selectedDomain;
	private PSNDTO selectedRow;
	private transient TreeNode<PSNNetNodeDTO> selectedNode;
	...
	private transient TreeNode<PSNTreeDTO> pseudonymTree;
	...
	public void load(String originalValue, String pseudonym)
	{
		...
		{
			autoShowTree = true;
			getPSNLazyModel().setForceEqualsMatchMode(true);
	...
	public void onSavePseudonym()
	{
		try
		{
			...
			if (multiPsnDomain || !originalValueAlreadyExists(originalValue))
			{
				String domainName = domain.getName();
				pseudonym = multiPsnDomain ?
						getServiceWithAutomaticNotification(domainName).createPseudonymFor(originalValue, domainName) :
						getServiceWithAutomaticNotification(domainName).getOrCreatePseudonymFor(originalValue, domainName);
	...
	public void onSavePseudonymPair()
	{
		...
		{
			boolean multiPsnDomain = domain.getConfig().isMultiPsnDomain();
			if ( (multiPsnDomain || !originalValueAlreadyExists(originalValue)) && !pseudonymAlreadyExists(pseudonym))
			{
				getServiceWithAutomaticNotification(domain.getName()).insertValuePseudonymPair(originalValue, pseudonym, domain.getName());
	...
	public void onDelete()
	{
		try
		{
			String domainName = selectedPseudonym.getDomainName();
			String psn = selectedPseudonym.getPseudonym();
			getServiceWithAutomaticNotification(domainName).deletePseudonym(psn, domainName);
			Object[] args = { psn, domainName };
		...
		catch (DeletionForbiddenException | UnknownDomainException | UnknownValueException | InvalidParameterException | InvalidPSNException | PSNNotFoundException e)
		{
			logMessage(e.getLocalizedMessage(), Severity.ERROR);
		}
	}
	...
	public void onAnonymise()
	{
		try
		{
			String domainName = selectedPseudonym.getDomainName();
			String psn = selectedPseudonym.getPseudonym();
			getServiceWithAutomaticNotification(domainName).anonymisePseudonym(psn, domainName);
			Object[] args = { selectedPseudonym.getOriginalValue(), psn, domainName };
			logMessage(new MessageFormat(getBundle().getString("edit.message.info.anonymised")).format(args), Severity.INFO, true, false);
			loadDomains();
			loadTree(psn);
		}
		catch (DBException | UnknownDomainException | InvalidParameterException | InvalidPSNException | PSNNotFoundException e)
		{
			logMessage(e.getLocalizedMessage(), Severity.ERROR);
		}
	}
	...
	private void loadTree(String value)
	{
		...
		try
		{
			net = getService().getPSNNetFor(value);

			// Create TreeNode for UI
			TreeNode<PSNTreeDTO> root = new DefaultTreeNode<>(net.getRoot());
	...
	public void onPseudonymiseOriginalValue()
	{
		...
		if (selectedPseudonym != null)
		{
			...
			DomainOutDTO current = getDomain(selectedPseudonym.getDomainName());
			if (current != null && current.getConfig().isMultiPsnDomain())
			{
				domain = current;
			}
			else
			{
				domain = getFirstRelatedDomain(Relation.SIBLING);
			}
		}
	}
	...
	public void onPseudonymisePseudonym()
	{
		...
		if (selectedPseudonym != null)
		{
			...
			domain = getFirstRelatedDomain(Relation.CHILD);
		}
	}

	public void setSelectedPseudonymFromTree()
	{
		selectedPseudonym = selectedNode == null ? null : selectedNode.getData().toPSNDTO();
	}
	...
	{
		return !StringUtils.isEmpty(originalValue) && getService().isAnonym(originalValue);
	}
	...
	public boolean isDeletable(PSNDTO psn) throws UnknownDomainException
	{
		if (psn != null && !ROOT_DOMAIN.equals(psn.getDomainName()))
		{
			try
			{
				return getManager().arePSNDeletable(psn.getDomainName());
			}
			catch (InvalidParameterException e)
			{
				logMessage(e.getLocalizedMessage(), Severity.ERROR);
	...
	private DomainOutDTO getFirstRelatedDomain(Relation relation)
	{
		return !getRelatedDomains(relation, true).isEmpty() ? getRelatedDomains(relation, true).getFirst() : null;
	}
	...
	public List<DomainOutDTO> getRelatedDomains(Relation relation, boolean message)
	{
		if (selectedPseudonym != null && StringUtils.isNotEmpty(selectedPseudonym.getDomainName()))
		{
			try
			{
				...
				DomainOutDTO currentDomain = getManager().getDomain(selectedPseudonym.getDomainName()); // @Arne: warum nicht einfach getDomain() ohne manager
				for (DomainOutDTO d : super.getDomains())
				{
					...
					else if (Relation.SIBLING.equals(relation) && (d.getConfig().isMultiPsnDomain() || !d.getName().equals(selectedPseudonym.getDomainName())))
					{
						// And if list of iterating domain parents exactly matches list of selected pseudonym domain parents
						Set<String> domainParents = new HashSet<>(currentDomain.getParentDomainNames());
						if (new HashSet<>(d.getParentDomainNames()).containsAll(domainParents) && domainParents.containsAll(d.getParentDomainNames()))
						{
							temp.add(d);
						}
					}
	...
	private boolean originalValueAlreadyExists(String originalValue) throws UnknownDomainException
	{
		try
		{
			String existingPseudonym = getService().getPseudonymFor(originalValue, domain.getName());
	...
	private boolean pseudonymAlreadyExists(String pseudonym) throws InvalidGeneratorException, InvalidPSNException, UnknownDomainException
	{
		try
		{
			String existingOriginalValue = getService().getValueFor(pseudonym, domain.getName());
	...
	private void searchNodes(TreeNode<PSNTreeDTO> parentNodeUI, PSNTreeDTO parentNodeDTO)
	{
		for (PSNTreeDTO childNodeDTO : parentNodeDTO.getChildren())
		{
			TreeNode<PSNTreeDTO> childNodeUI = new DefaultTreeNode<>(childNodeDTO, parentNodeUI);
	...
	public void loadDomains()
	{
		...
		if (selectedDomain != null)
		{
			setSelectedDomain(getDomains().stream().filter(d -> selectedDomain.getName().equals(d.getName())).findAny().orElse(selectedDomain));
		}
	}
	...
	public List<DomainOutDTO> getDomainsWithAll()
	{
		...
		all.setNumberOfPseudonyms(
				(int) getAllDomainsSortedByLabel().stream().mapToLong(DomainOutDTO::getNumberOfPseudonyms).sum());
	...
	public PSNDTOLazyModel getPSNLazyModel()
	{
		...
		{
			psnLazyModel = new PSNDTOLazyModel(getManager(), getSelectedDomains());
		}
		else
		{
	...
	public TreeNode<PSNTreeDTO> getPseudonymTree()
	{
		return pseudonymTree;
	}

	public TreeNode<PSNNetNodeDTO> getSelectedNode()
	{
		return selectedNode;
	}

	public void setSelectedNode(TreeNode<PSNNetNodeDTO> selectedNode)
	{
