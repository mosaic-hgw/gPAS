...
public class Generator
{
	...
	public Generator(String checkDigitClassName, Alphabet alphabet, DomainConfig config, String domainName)
			throws InvalidGeneratorException, InvalidCheckDigitClassException
	{
		this(CheckDigits.createCheckDigitClass(checkDigitClassName), alphabet, config, domainName);
	}

	public Generator(Class<? extends CheckDigits> checkDigitClass, Alphabet alphabet, DomainConfig config, String domainName)
			throws InvalidGeneratorException, InvalidCheckDigitClassException
	{
		this(CheckDigits.createCheckDigits(checkDigitClass, alphabet, config), alphabet, config, domainName);
	}

	public Generator(CheckDigits checkDigits, Alphabet alphabet, DomainConfig config, String domainName)
	...
	public CheckDigits getCheckDigits()
	{
		return checkDigits;
	}
	 ...
	 */
	public String getNewPseudonym(long number) throws InvalidParameterException
	{
		if (logger.isTraceEnabled())
		{
			logger.trace("generate new pseudonym");
		}
		...
		catch (CharNotInAlphabetException | InvalidParameterException e)
		{
			logger.error("this should never happen - must be due to a misconfigured domain", e);
			if (e instanceof InvalidParameterException ipe)
			{
				throw ipe;
			}
			throw new InvalidParameterException(e);
		}
		logger.trace("new pseudonym: {}", result);
		return result;
	}
