...
/**
 * validates a parent domain String at creation of new domains; checks if the parent domain exists
 * 
 * @author Robert Wolff
 * 
 */
@ManagedBean(name = "parentDomainValidator")
@RequestScoped
public class ParentDomainValidiator implements Validator {
	@EJB(lookup = "java:global/gpas/psn-ejb/DomainManagerBean!org.emau.icmvc.ganimed.ttp.psn.DomainManager")
	private DomainManager manager;

	private final Logger logger = LoggerFactory.getLogger(ParentDomainValidiator.class);

	@Override
	public void validate(FacesContext arg0, UIComponent arg1, Object value) throws ValidatorException {
		boolean exists = false;
		if (logger.isDebugEnabled()) {
			logger.debug("validating parent domain");
		}
		...
		if (value != null && !((String) value).isEmpty()) {
			...
			if (!exists) {
				if (logger.isDebugEnabled()) {
					logger.debug("validation failed: parent domain doesn't exist");
				}
				throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "validation error: parent domain doesn't exist",
