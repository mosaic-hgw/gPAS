...
public abstract class CheckDigitTests {
	...
	private static final Logger logger = Logger.getLogger(CheckDigitTests.class);
	...
	protected void testTransposition(String messageWithCheckDigits, int length) throws Exception {
		for (int i = 0; i < messageWithCheckDigits.length() - 2; i++) {
			if (messageWithCheckDigits.charAt(i + 1) != messageWithCheckDigits.charAt(i + 1)) {
			...
			for (int j = 1; j < alphabet.length(); j++) {
				...
				for (int k = i + 1; k < messageWithCheckDigits.length(); k++) {
					for (int l = 1; l < alphabet.length(); l++) {
						char errorneousChar2 = alphabet.getSymbol((alphabet.getPosForSymbol(messageWithCheckDigits.charAt(k)) + l)
								% alphabet.length());
